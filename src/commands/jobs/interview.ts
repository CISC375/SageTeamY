...
export default class extends Command {
	description =
		"Practice answering common interview questions for a particular job role.";
	...
	async run(
		...
		await interaction.reply({
			content: "Choose a job role to practice interview questions:",
			components: [row],
		});
...
export async function handleInterviewOptionSelect(
	i: StringSelectMenuInteraction
): Promise<void> {
	// get the dropdown selection
	const choice = i.values?.[0];
	if (!choice) {
		await i.reply({ content: "No job role selected.", ephemeral: true });
		return;
	}

	// start a DM conversation with the user
	let dmChannel;
	try {
		dmChannel = await i.user.createDM();
		await dmChannel.send(
			"Starting your practice interview. You will receive 5 questions. You will have 5 minutes per question."
		);
		await i.reply({
			content: "Check your DMs for the interview.",
			ephemeral: true,
		});
	} catch (err) {
		await i.reply({
			content: "I couldn't DM you. Maybe check your privacy settings?",
			ephemeral: true,
		});
		return;
	}
	...
	if (choice === "software_engineer") {
		questions = questions.concat(softwareDevEng);
	} else if (choice === "systems_administrator") {
		questions = questions.concat(sysAdmin);
	} else if (choice === "data_scientist") {
		questions = questions.concat(dataScientist);
	} else if (choice === "ui_ux_designer") {
		questions = questions.concat(uxuiDesigner);
	} else if (choice === "it_project_manager") {
		questions = questions.concat(projManager);
	}

	// shuffle all the questions and pick the first five
	const pool = questions.slice();
	for (let index = pool.length - 1; index > 0; index--) {
		const swap = Math.floor(Math.random() * (index + 1));
		[pool[index], pool[swap]] = [pool[swap], pool[index]];
	}
	const selectedQuestions = pool.slice(0, 5);

	const answers: string[] = [];

	for (let qIndex = 0; qIndex < selectedQuestions.length; qIndex++) {
		const questionText = `Question ${qIndex + 1}/5: ${
			selectedQuestions[qIndex]
		}`;
		await dmChannel.send(questionText);

		// collect the user's responses, 5 minutes per question
		try {
			...
			// if they don't reply, end the interview
			const reply = collected.first();
			if (!reply) {
				await dmChannel.send(
					"No answer received. Ending the interview."
				);
				break;
			}
			...
			await dmChannel.send(
				"Time expired waiting for a reply. Interview ended."
			);
			break;
		}
	}
	await dmChannel.send("Interview complete.");
}
